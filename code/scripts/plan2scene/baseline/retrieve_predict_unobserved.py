...
def compute_rs_statistics(conf: ConfigManager, textures: dict):
    ...
    # {k:{v: [] for v in conf.surfaces} for k in conf.room_types}
    train_rs_closest_picks = {}
    for i, (house_key, house) in enumerate(train_houses.items()):
        logging.info("[%d/%d] Processing train set: %s" %
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Predict textures for unobserved surfaces using the retrieve approach.")
    ...
    parser.add_argument(
        "smt_path", help="Path to substance mapped textures dataset")
