    ...
    for room_index, room in house.rooms.items():
        ...
        for surface in conf.surfaces:
            if len(room.surface_embeddings[surface]) > 0:
                room.surface_embeddings[surface] = {"prop": torch.mean(torch.cat(
                    list(room.surface_embeddings[surface].values())), dim=0).unsqueeze(0)}
            else:
                room.surface_embeddings[surface] = {}

    fill_textures(conf, {house.house_key: house}, predictor=predictor, log=False,
                  image_source=ImageSource.MEAN_EMB, skip_existing_textures=False)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Predict textures for observed surfaces using the naive synth approach.")
    ...
    for i, (house_key, house) in enumerate(houses.items()):
        ...
        save_house_crops(house, osp.join(
            output_path, "texture_crops", house_key))
        save_house_texture_embeddings(house, osp.join(
            output_path, "surface_texture_embeddings", house_key + ".json"))
