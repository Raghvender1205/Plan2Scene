...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Predict textures for observed surfaces using the retrieve approach.")
            ...
            for prop_index in [x for x in range(0, multiprop)] + [-1]:
                ...
                for surface in conf.surfaces:
                    if len(crop_room.surface_textures[surface]) > 0:
                        ...
                        medoid_crop = crop_room.surface_textures[surface][get_medoid_key(
                            crop_room.surface_textures[surface])]
                        output_room.surface_textures[surface][prop_key] = find_closest_match(
                            conf, medoid_crop, textures)

        save_house_crops(output_house, save_path=osp.join(
