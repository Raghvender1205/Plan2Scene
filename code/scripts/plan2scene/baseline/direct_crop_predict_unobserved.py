from src.plan2scene.common.house_parser import parse_houses, load_houses_with_textures, map_surface_crops_to_houses, save_house_crops
from src.plan2scene.common.residence import Room, House
from src.plan2scene.config_manager import ConfigManager
import logging

from src.plan2scene.utils.image_util import get_medoid_key
            ...
            if len(room.surface_textures[surface]) > 0:
                if "prop" in room.surface_textures[surface]:
                    house_rs_crops[room_type][surface].append(
                        room.surface_textures[surface]["prop"])
                else:
                    house_rs_crops[room_type][surface].append(
...
def collect_train_set_crops(conf: ConfigManager) -> dict:
    ...
    for i, (house_key, house) in enumerate(train_houses.items()):
        logging.info("[%d/%d] Processing TrainSet: %s" %
                     (i, len(train_houses), house_key))
...
if __name__ == "__main__":
    ...
    # Mapping room_type -> surface_type -> textures
    train_rs_crops = collect_train_set_crops(conf)
    # Mapping surface_type -> textures
    train_s_crops = {"floor": [], "wall": [], "ceiling": []}
    for _, surfaces in train_rs_crops.items():
        for surface in surfaces:
            train_s_crops[surface].extend(surfaces[surface])

    # Load houses
    houses = load_houses_with_textures(
        conf, data_split=split, drop_fraction=conf.drop_fraction, textures_path=observed_surface_textures_path)
    ...
    for i, (house_key, house) in enumerate(houses.items()):
        ...
        for room_index, room in house.rooms.items():
            ...
            # -1 is the base case. It must be added to the end to ensure key is populated last.
            for prop_index in [x for x in range(0, multiprop)] + [-1]:
                ...
                for surface in conf.surfaces:
                    if key not in room.surface_textures[surface]:
                        ...
                        room.surface_textures[surface][prop_key] = pick_texture(
                            train_rs_crops, train_s_crops, house_rs_crops, room_type, surface)
                    else:
                        # Multiprop
                        room.surface_textures[surface][prop_key] = room.surface_textures[surface][key]

        save_house_crops(house, save_path=osp.join(
            output_path, "texture_crops", house_key))
