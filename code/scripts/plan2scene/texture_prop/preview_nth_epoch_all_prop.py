...
if __name__ == "__main__":
    ...
    parser.add_argument("output_path", type=str,
                        help="Output directory which will contain the preview.html file.")
    parser.add_argument("interval", type=int,
                        help="Interval between previewed epochs.")
    ...
    with open(osp.join(output_path, "preview.html"), "w") as f:
        ...
        while True:
            candidates = glob.glob(
                osp.join(checkpoints_path, f"loss-*-epoch-{current_checkpoint_id}.ckpt"))
            if len(candidates) == 0:
                break
            ...
            command = "/bin/bash ./code/scripts/plan2scene/texture_prop/preview_all_prop.sh {output_path} {conf_path} {checkpoint_path}".format(
                output_path=osp.join(
                    output_path, f"epoch_{current_checkpoint_id}"),
                conf_path=conf_path,
            ...
            if preview_images is None:
                preview_images = os.listdir(
                    osp.join(output_path, f"epoch_{current_checkpoint_id}", "archs"))
                preview_images = [
                    a for a in preview_images if a.endswith(".png")]
                f.write("<tr>\n")
                f.write("<th>Epoch</th>\n")
            ...
            for preview_image in preview_images:
                f.write("<td>\n")
                f.write("<img src='{img_path}' style='width:200px;'/>".format(
                    img_path=osp.join(f"epoch_{current_checkpoint_id}", "archs", preview_image)))
                f.write("</td>\n")
