...
class MLP(nn.Module):
    def __init__(self, param, model_param):
        ...
        self.first_conv = Conv2dBlock(self.nf_in, self.n_featutres, 1, 1, 0, None,
                                      model_param.non_linearity, model_param.dropout_ratio, bias=self.bias)

        self.res_blocks = nn.ModuleList()

        for idx in range(self.n_blocks):
            block_i = Conv2dBlock(self.n_featutres, self.n_featutres, 1, 1, 0, None,
                                  model_param.non_linearity, model_param.dropout_ratio, bias=self.bias)
            self.res_blocks.append(block_i)

        self.last_conv = Conv2dBlock(self.n_featutres, self.nf_out, 1,
                                     1, 0, None, None, model_param.dropout_ratio, bias=self.bias)
