...
class AbstractTrainer(abc.ABC):
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.max_epoch + 1):
            ...
            checkpoint_metric_value = self._get_checkpoint_metric_value(
                val_stats)
            if self._best_checkpoint_metric_value is None or checkpoint_metric_value < self._best_checkpoint_metric_value:
                ...
                self._save_checkpoint(
                    epoch, SaveReason.BEST_MODEL, train_stats, val_stats)

            if epoch % self.save_model_interval == 0:
                self._save_checkpoint(
                    epoch, SaveReason.INTERVAL, train_stats, val_stats)
