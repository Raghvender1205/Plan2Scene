...
def ray_test(mask, ray_origin: tuple, ray_direction: tuple, ray_length: float) -> tuple:
    ...
    for hit_point in hit_points:
        d = math.sqrt((hit_point[0] - ray_origin[0]) **
                      2 + (hit_point[1] - ray_origin[1]) ** 2)
...
def get_room_mask(room: Room):
    ...
    for wall in room.walls:
        ...
        ls = LineString([(wall.p1[0], wall.p1[2]),
                         (wall.p2[0], wall.p2[2])]).buffer(1)
        ...
        for hole in wall.holes:
            ...
            hs = LineString(
                [(h_start_x, h_start_y), (h_end_x, h_end_y)]).buffer(1)
            ls = ls.difference(hs)
...
def get_transform(x: float, y: float, z: float, angle: float, scale_x: float = 1, scale_z: float = 1) -> list:
    ...
    scale_mat = np.array([[scale_x, 0, 0, 0], [0, 1, 0, 0], [
                         0, 0, scale_z, 0], [0, 0, 0, 1]], dtype=float)
...
def point_line_seg_distance(p1: tuple, p2: tuple, p: tuple, extend_line: bool = False) -> tuple:
    ...
    x2, y2 = p2
    x3, y3 = p
    x1, x2, x3, y1, y2, y3 = float(x1), float(
        x2), float(x3), float(y1), float(y2), float(y3)
    px = x2 - x1
    py = y2 - y1
