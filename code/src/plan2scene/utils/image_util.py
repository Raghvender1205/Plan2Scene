...
def get_medoid_key(textures: dict) -> str:
    ...
    for crop_key, crop_image in textures.items():
        ...
        texture_vectors.append(torchvision.transforms.ToTensor()(
            crop_image.image).view(1, -1))
    texture_vectors = torch.cat(texture_vectors, dim=0)
    mean_vector = texture_vectors.mean(dim=0)
    delta_vectors = texture_vectors - \
        mean_vector.unsqueeze(0).repeat(texture_vectors.shape[0], 1)
