...
class Room(object):
    ...
    def flush(self, flush_key: str) -> None:
        ...
        for surface in self.surface_textures:
            if flush_key in self.surface_textures[surface]:
                self._arch_room.assign_texture(
                    surface, self.surface_textures[surface][flush_key].image)
            else:
    ...
    def from_arch_room(cls, house_key: str, room_index: int, arch_room: arch_parser.models.room.Room, surfaces: list):
        ...
        room = Room(house_key=house_key,
                    room_index=room_index, surfaces=surfaces)
        ...
        for k1, _, k2 in arch_house.door_connected_room_pairs:
            if k2 is None:
                house.door_connected_room_pairs.append(
                    (room_id_room_index_map[k1], -1))
            else:
                house.door_connected_room_pairs.append(
                    (room_id_room_index_map[k1], room_id_room_index_map[k2]))
