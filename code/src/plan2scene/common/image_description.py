...
class ImageSource(Enum):
    ...
    # The image is synthesized based on VGG Crop Selects suggestion (Output for observed surfaces)
    VGG_CROP_SELECT = ("VGGCropSelect", True)
    # The image is a synthesized based on GNNs suggestions (Output for unobserved surfaces)
    GNN_PROP = ("GNNProp", False)
    # The image is synthesized by multiprop script.
    MULTI_PROP = ("MultiProp", None)
    # The image is chosen by the retrieve baseline for an observed surface
    RETRIEVE_OBSERVED = ("RetrieveObserved", True)
    # This image is chosen by the retrieve baseline for an unobserved surface
    RETRIEVE_UNOBSERVED = ("RetrieveUnobserved", False)
    # Image synthesized from a mean embedding crop
    MEAN_EMB = ("MeanEmb", True)
    # Image synthesized from a room type and surface type conditioned mean embedding crop
    RS_MEAN_EMB = ("RSMeanEmb", True)
