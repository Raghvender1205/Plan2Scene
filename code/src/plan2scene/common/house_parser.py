...
from arch_parser.prefered_format import PreferredFormat
...
def parse_houses(conf: ConfigManager, house_keys, house_path_spec: str, photoroom_csv_path_spec: str) -> dict:
    ...
    for house_key in house_keys:
        ...
        results[house_key] = House.from_arch_house(
            arch_house, surfaces=conf.surfaces)
...
def map_surface_crops_to_house(conf: ConfigManager, house: House) -> None:
    ...
    """
    for room_indes, room in house.rooms.items():
        assert isinstance(room, Room)
        for photo in room.photos:
            for surface in conf.surfaces:
                surface_instances = [i for i in range(
                    conf.texture_gen.masks_per_surface[surface])]
...
def save_house_crops(house: House, save_path: str) -> None:
    ...
    assert not osp.exists(
        save_path), f"Already saved. Please delete {save_path}"
    ...
    for room_index, room in house.rooms.items():
        ...
        for surface, crop_map in room.surface_textures.items():
            ...
            for crop_name, crop_img_description in crop_map.items():
                ...
                crop_img_description.save(
                    osp.join(save_path, room.room_id, surface, crop_name + ".png"))
...
def load_house_crops(conf: ConfigManager, house: House, save_path: str, exclude_prior_path: str = None, restrict_prior_path: str = None) -> None:
    ...
    for room_index, room in house.rooms.items():
        ...
        assert osp.exists(osp.join(save_path, room.room_id)
                          ), "Not exist %s" % osp.join(save_path, room.room_id)

        for surface in conf.surfaces:
            # import pdb; pdb.set_trace()
            if osp.exists(osp.join(save_path, room.room_id, surface)):
                if exclude_prior_path is not None:  # Exclude prior predictions
                    assert osp.exists(osp.join(exclude_prior_path, room.room_id, surface)
                                      ), "Prior path doesn't have a matching room surface."
                    prior_files = os.listdir(
                        osp.join(exclude_prior_path, room.room_id, surface))
                    prior_files = [
                        a for a in prior_files if a.endswith(".png")]
                ...
                if restrict_prior_path is not None:  # Restrict to prior predictions
                    assert osp.exists(osp.join(restrict_prior_path, room.room_id, surface)
                                      ), "Prior path doesn't have a matching room surface."
                    prior_files = os.listdir(
                        osp.join(restrict_prior_path, room.room_id, surface))
                    prior_files = [
                        a for a in prior_files if a.endswith(".png")]
                    assert len(prior_files) <= 1, "Multiple prior predictions."
                    if len(prior_files) == 1:
                ...
                crop_files = os.listdir(
                    osp.join(save_path, room.room_id, surface))
                crop_files = [a for a in crop_files if a.endswith(".png")]
                for crop_file in crop_files:
                    crop_name = os.path.splitext(crop_file)[0]
                    crop_image_description = ImageDescription.parse_image(
                        osp.join(save_path, room.room_id, surface, crop_file))
