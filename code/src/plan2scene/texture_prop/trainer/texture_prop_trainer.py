...
class TexturePropTrainer(AbstractTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Used to inform the dataloaders to refresh data.
        self._epoch_counter = multiprocessing.Value("i", 0)
    ...
    def _setup_datasets(self) -> None:
        ...
        """
        train_graph_generator = get_graph_generator(
            self.conf, self.system_conf.train_graph_generator, include_target=True)
        ...
        self._val_dataloader = DataLoader(
            self._val_dataset, batch_size=self.system_conf.train.bs)
        ...
        self._val_nt_dataloader = DataLoader(
            self._val_nt_dataset, batch_size=self.system_conf.train.bs)
    ...
    def _setup_extra(self) -> None:
        ...
        self._tg_predictor.load_checkpoint(
            checkpoint_path=self.conf.texture_gen.checkpoint_path)
    ...
    def _setup_metrics(self) -> list:
        ...
        return [
            ...
            MetricDescription("subs", PairedMatcher(ClassificationError(
                SubstanceClassifier(classifier_conf=self.conf.metrics.substance_classifier)))),
            MetricDescription("freq", PairedMatcher(FreqHistL1())),
            MetricDescription("tile", UnpairedMatcher(TileabilityMean(
                metric_param=self.conf.metrics.tileability_mean_metric))),
        ]
    ...
    def _train_epoch(self, epoch: int) -> EpochSummary:
        ...
        epoch_summary = EpochSummary(
            epoch_loss=0.0, epoch_entry_count=0, epoch_batch_count=0)
        for i, batch in enumerate(self.train_dataloader):
            self.optim.zero_grad()

            output = self.net(batch.to(self.device))
            mask_repeated = batch.y_mask.unsqueeze(
                -1).repeat([1, 1, self._combined_emb_dim])
    ...
    def _deep_eval_epoch(self, epoch) -> list:
        ...
        fill_textures(self.conf, pred_houses, log=False, predictor=self._tg_predictor,
                      image_source=ImageSource.GNN_PROP, skip_existing_textures=False)

        # Load ground truth
        gt_houses = load_houses_with_textures(
            self.conf, "val", "0.0", self.conf.data_paths.gt_reference_crops_val)
        ...
        for metric in self.metrics:
            ...
            result = evaluate(self.conf, pred_houses=pred_houses,
                              gt_houses=gt_houses, matcher=metric.evaluator, log=False)
    ...
    def _report(self, epoch: int, train_stats: EpochSummary, val_stats: EpochSummary, deep_eval_results: list) -> None:
        ...
        if deep_eval_results:
            for deep_eval_result in deep_eval_results:
                ...
                additional += "%s: %.7f [%.7f/%d]\t" % (
                    ...
                    deep_eval_result.eval_result.total_texture_loss /
                    deep_eval_result.eval_result.surface_count,
        ...
        self.summary_writer.add_scalar(
            "epoch_train_loss", train_stats.epoch_loss / train_stats.epoch_batch_count, epoch)
        self.summary_writer.add_scalar(
            "epoch_val_loss", val_stats.epoch_loss / val_stats.epoch_batch_count, epoch)
