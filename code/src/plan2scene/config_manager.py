...
class ConfigManager:
    def __init__(self):
        ...
        # Configuration used to parse house architectures.
        self.house_gen = None
        # Configuration used to synthesize textures for observed surfaces.
        self.texture_gen = None
        # Configuration used to propagate textures for unobserved surfaces.
        self.texture_prop = None
        ...
        # Fraction of surfaces synthetically unobserved.
        self.drop_fraction = None
        # Configuration used to render houses using scene toolkit.
        self.render_config = None
        # Configuration used for seam correction of textures.
        self.seam_correct_config = None
    ...
    def copy_config_to_output(self) -> None:
        ...
        shutil.copyfile(self.args.texture_prop, osp.join(
            self.output_path, "conf", "texture_prop.json"))
        shutil.copyfile(self.args.texture_gen, osp.join(
            self.output_path, "conf", "texture_gen.json"))
    ...
    def process_args(self, args, output_is_dir=False) -> None:
        ...
        self.surfaces = parse_config(
            osp.join(args.labels_path, "surfaces.json"))
        self.room_types = parse_config(
            osp.join(args.labels_path, "room_types.json"))

        self.texture_prop.node_embedding_dim = len(
            self.room_types) + 3 * (self.texture_gen.combined_emb_dim + 1)
        self.texture_prop.node_target_dim = 3 * \
            (self.texture_gen.combined_emb_dim)
    ...
    def add_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        """
        parser.add_argument(
            "--seed", help="Default seed value to use.", default=12415, type=int)
        ...
        parser.add_argument("--metric-conf", help="Path to metric.json.",
                            default="./conf/plan2scene/metric.json")
        ...
        parser.add_argument("--seam-correct-config", help="Path to ./conf/plan2scene/seam_correct.json",
                            default="./conf/plan2scene/seam_correct.json")
        ...
        parser.add_argument("--num-workers", default=4, type=int,
                            help="Number of workers used by a data loader.")
