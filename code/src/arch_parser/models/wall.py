class Wall:
    ...
    def __init__(self, wall_id: str, p1: tuple, p2: tuple, extra_args):
        assert isinstance(p1, tuple) and len(p1) == 3 and all(
            [isinstance(a, float) for a in p1])
        assert isinstance(p2, tuple) and len(p2) == 3 and all(
            [isinstance(a, float) for a in p2])
