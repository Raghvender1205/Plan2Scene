...
def serialize_texture(texture: Image, surface_id: str, materials_json: list, textures_json: list,
    ...
    mat_json = {"uuid": f"Material_{surface_id}",
                "map": f"Texture_{surface_id}"}
    materials_json.append(mat_json)

    tex_json = {"uuid": f"Texture_{surface_id}",
                "image": f"Image_{surface_id}"}
...
def serialize_arch_json(house: House) -> dict:
    ...
    for room_id, room in house.rooms.items():
        rooms.append(serialize_room(room, elements,
                                    materials_json, textures_json, images_json))
