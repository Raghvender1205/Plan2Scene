...
def parse_rdr(rdr_json: list, house: House) -> None:
    ...
    for from_room_id, hole_id, to_room_id in rdr_json:
        house.door_connected_room_pairs.append(
            (from_room_id, hole_id, to_room_id))
...
def parse_material(material_entry: dict, arch_json: dict) -> Image.Image:
    ...
    mat_entry = [a for a in arch_json["materials"]
                 if a["uuid"] == material_entry["materialId"]]
    assert len(mat_entry) == 1
    mat_entry = mat_entry[0]

    # Parse texture entry
    texture_entry = [a for a in arch_json["textures"]
                     if a["uuid"] == mat_entry["map"]]
    assert len(texture_entry) == 1
    texture_entry = texture_entry[0]

    # Parse image entry
    image_entry = [a for a in arch_json["images"]
                   if a["uuid"] == texture_entry["image"]]
...
def parse_floor_element(element_json: dict, arch_json: dict, house: House) -> None:
    ...
    extra_args = {k: v for k, v in element_json.items(
    ) if k not in ["points", "roomId", "id", "type", "materials"]}
    floor = Floor(floor_id=element_json["id"],
                  points=floor_points, extra_args=extra_args)

    room = house.rooms[room_id]
    assert isinstance(room, Room)

    if len(element_json["materials"]) > 0:
        room.floor_texture = parse_material(
            element_json["materials"][0], arch_json)
...
def parse_hole(hole_json: dict, wall: Wall) -> None:
    ...
    extra_args = {k: v for k, v in hole_json.items() if k not in [
        "box", "id", "type"]}
...
def parse_wall_element(element_json: dict, arch_json: dict, house: House) -> None:
    ...
    extra_args = {k: v for k, v in element_json.items(
    ) if k not in ["points", "holes", "roomId", "id", "type", "materials"]}
    wall = Wall(wall_id=element_json["id"], p1=tuple(
        wall_points[0]), p2=tuple(wall_points[1]), extra_args=extra_args)
    ...
    if len(element_json["materials"]) > 0:
        ...
        room.wall_texture = parse_material(
            element_json["materials"][0], arch_json)
...
def parse_ceiling_element(element_json: dict, arch_json: dict, house: House) -> None:
    ...
    ceiling = Ceiling(
        ceiling_id=element_json["id"], points=ceiling_points, extra_args=extra_args)

    room = house.rooms[room_id]
    assert isinstance(room, Room)

    if len(element_json["materials"]) > 0:
        room.ceiling_texture = parse_material(
            element_json["materials"][0], arch_json)
